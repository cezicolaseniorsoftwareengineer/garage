"""Generates test_battery.js — run: python scripts/gen_battery.py"""
import os, textwrap
...
js = textwrap.dedent(f"""\
    ...
    (function () {{
        ...
        if (s < 0 {OR} e < 0) {{ console.error('FATAL: JavaAnalyzer not found s=' + s + ' e=' + e); process.exit(1); }}
    ...
    (function () {{
        ...
        if (s < 0 {OR} e < 0 {OR} mi < 0) {{ console.error('FATAL: CODE_CHALLENGES not found s=' + s + ' e=' + e + ' mi=' + mi); process.exit(1); }}
    ...
    (function () {{
        ...
        var mi = src.indexOf(mk, s);
        var e = src.lastIndexOf('}};\\n', mi);
        if (s < 0 {OR} e < 0 {OR} e <= s) {{ console.error('FATAL: SCALE_MISSIONS not found s=' + s + ' e=' + e + ' mi=' + mi); process.exit(1); }}
    ...
    Object.keys(SCALE_MISSIONS).forEach(function(mId) {{
        ...
        mission.steps.forEach(function(step, si) {{
            ...
            if (!step.validator) {{
                ...
                if (baR.ok {AND} bvR.ok) {{ console.log('  [PASS] ' + lbl + ' (base)'); passed++; }}
                else {{
...
out = os.path.join(os.path.dirname(__file__), '..', 'test_battery.js')
