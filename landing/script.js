...
// ── CHECKOUT LINKS (env-driven: generated by Netlify build via landing/env.js)
// Use `scripts/write_netlify_env.js` in build to create `landing/env.js` from Netlify env vars.
const CHECKOUT_LINKS = (typeof window !== 'undefined' && window.__ENV__) ? {
    mensal: window.__ENV__.ASAAS_MONTHLY || 'https://www.asaas.com/c/k8xqrulte259faq2',
    anual: window.__ENV__.ASAAS_ANNUAL || 'https://www.asaas.com/c/6l46uwz5qxlygofc',
} : {
    // Fallback to test links when env not present (local/dev)
